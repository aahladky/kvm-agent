...
void ph_cmd_kbd_send_key(const u8 *args) { // 2 bytes
	// RP2350 port: args[0] is a RAW USB HID usage code (not PiKVM's own keymap ID) --
	// the host-side bridge already owns a name->USB-HID-usage table (ported from the
	// prior CircuitPython/adafruit_hid firmware), so skip ph_usb_keymap's indirection
	// rather than maintaining two parallel keycode tables.
	const u8 key = args[0];
	if (key > 0) {
		if (PH_O_IS_KBD_USB) {
