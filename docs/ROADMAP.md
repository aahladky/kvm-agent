...
1. **Phase 0 firmware hardening** — watchdog + lock-step host discipline. Cheap, high-leverage, protects every future long run. **CODE LANDED 2026-07-22** (`docs/SESSION_2026-07-22_slice_b_firmware_hardening.md`) — watchdog, host retry, mouse-suspend retain+resend, PONG visibility bits, `tools/soak.py`. **Gate PENDING**: BOOTSEL flash + Pi 5 deploy + the overnight soak itself, operator-driven.
