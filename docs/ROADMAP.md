...
5. **Phase 2 — subgoal unit + independent verification.** **PLANNED 2026-07-22, NOT EXECUTED** (`docs/PLAN_2026-07-22_phase2_subgoal_verification.md`): four measurement-gated slices, each its own branch — **D-a** the postcondition oracle + an offline replay eval against the existing graded run archive (no rig time; the go/no-go on whether a Holo-based oracle works at all), **D-b** shadow wiring + 3-4 longer battery tasks + `battery_metrics.py` (one rig session buys the flat-loop baseline, the false-refusal rate, the grading-agreement number, and the probe on whether the model plans on longer tasks), **D-c** flip both gates on — in-loop terminal gating (k-strikes → run ends FAILED, loudly) and battery auto-grading with a defined human ground-truth sample, **D-d** the subgoal unit with the plan scoping what the actor sees. Two measured facts drove the ordering: `update_plan` is unused (§2 above), and the battery is at its ceiling (three consecutive all-pass runs, zero archived false-"finished"), so Phase 2's own gate is unmeasurable until the task list grows.
