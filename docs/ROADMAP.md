...
- [ ] No hang watchdog in `main()` — enable HW watchdog + pet it, or the whole input channel can wedge silently mid-task.
- [ ] UART framing resyncs only on idle gaps → enforce **lock-step host discipline** (send → await PONG → retry) so the host's own pause creates the resync gap. Buys confirmation *and* self-healing framing in one rule. *[correction 2026-07-22: half done — the host already blocks send→await-PONG per command under a lock (`pikvm_proto._roundtrip`); only the retry-on-failure half is missing.]*
...
- **`update_plan` is decorative** — telemetry, not control. Nothing in the loop sequences/verifies/recovers off it.
...
0. *[added 2026-07-22, per §0's own rule — measurement forces this piece first:]* **TOCTOU pre-fire guard + tool-result magnitude/region** — the first complete battery measured target-side async re-flow vs the ~15s step cadence as the dominant failure source. Host-side only, offline-testable, the runtime slice of Phase 2's "verification stops being self-judged." **DONE 2026-07-22** (`agent_loop_holo.py`, `docs/SESSION_2026-07-22_roadmap_alignment.md`); rig confirmation via the next battery rerun still pending.
1. **Phase 0 firmware hardening** — watchdog + lock-step host discipline. Cheap, high-leverage, protects every future long run. **PENDING** — Slice B of `docs/PLAN_2026-07-22_roadmap_alignment_slices.md` Part 3, own branch, needs rig time (BOOTSEL flash + overnight soak).
