...
def build_backend(mock: bool, cfg: dict):
    ...
    # repo layout: patched agent at repo root, evocua/ provides mm_agents.* submodules.
    # APPEND evocua/ (don't insert(0)): repo root must stay ahead so `import evocua_agent` loads the
    # PATCHED root copy (5/5 tool_call formats + answer-channel), not the stale evocua/evocua_agent.py
    # (1/5, no answer-channel -> the 2-way street would never fire on the real backend). See DEMOS.md.
    here = os.path.dirname(os.path.abspath(__file__))
    if here not in sys.path:
        sys.path.insert(0, here)
    repo = os.path.join(here, "evocua")
    if repo not in sys.path:
        sys.path.append(repo)
    from evocua_agent import EvoCUAAgent          # patched copy at repo root (evocua/ copy is shadowed)
