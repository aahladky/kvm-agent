...
sys.path.append(REPO)   # APPEND (not insert(0)): evocua/ provides mm_agents.* submodules, but the
                        # repo-root dir must stay ahead of it so `import evocua_agent` loads our
                        # PATCHED root copy, not the stale evocua/evocua_agent.py (which parses 1/5
                        # tool_call formats and has no answer-channel). See DEMOS.md.
...
from evocua_agent import EvoCUAAgent   # our patched copy at repo root (evocua/evocua_agent.py is shadowed)   # noqa: E402  # type: ignore[import]
